...
func verifyStringRegen(t *testing.T, in, out *corev1.Secret) {
	...
	if len(regenKeys) != 0 {
		for _, key := range regenKeys {
			...
			if bytes.Equal(in.Data[key], val) {
				t.Errorf("key %s was not regenerated", key)
				continue
			}
		}
	}
	...
	for _, key := range genKeys {
		...
		t.Errorf("key %s was unexpectedly regenerated", key)
	}
}
...
func TestRegenerateInsecureSingleFieldSecureBefore(t *testing.T) {
	...
	if !reflect.DeepEqual(in.Data, out.Data) {
		t.Fatal("secure generated data changed")
	}
	if out.Annotations[secret.AnnotationSecretSecure] != "yes" {
		t.Fatal("secure marker was removed")
	}
}
...
func enableRegenerateInsecure(t *testing.T) {
	t.Helper()
	viper.Set("regenerate-insecure", true)
	t.Cleanup(func() { viper.Set("regenerate-insecure", false) })
}
...
func TestUniqueness(t *testing.T) {
	...
	require.NoError(t, mgr.GetClient().Get(context.TODO(), types.NamespacedName{
		Name:      in.Name,
		Namespace: in.Namespace}, out))
	require.Equal(t, resourceVersion, out.ResourceVersion)
	if !reflect.DeepEqual(in.Annotations, out.Annotations) {
		t.Fatal("invalid request changed Secret annotations")
	}
	if !reflect.DeepEqual(in.Data, out.Data) {
		t.Fatal("invalid request changed Secret data")
	}
}
