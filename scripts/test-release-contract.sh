...
git -C "$tmp" tag v4.0.0-rc.5
run v4.0.0-rc.5 "$sha"
if run v4.0 "$sha" >/dev/null 2>&1; then echo 'invalid tag accepted' >&2; exit 1; fi
if run v4.0.0-rc.5 aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa >/dev/null 2>&1; then echo 'wrong source accepted' >&2; exit 1; fi

sed -i.bak 's/^version: 4.0.0-rc.5$/version: 4.0.0/; s/^appVersion: v4.0.0-rc.5$/appVersion: v4.0.0/' "$tmp/deploy/helm-chart/kubernetes-secret-generator/Chart.yaml"
sed -i.bak 's/tag: v4.0.0-rc.5/tag: v4.0.0/' "$tmp/deploy/helm-chart/kubernetes-secret-generator/values.yaml"
git -C "$tmp" add .
...
identity=https://github.com/mrchypark/kubernetes-secret-generator/.github/workflows/release-candidate.yml@refs/tags/v4.0.0-rc.5
sh scripts/verify-signing-identity.sh "$identity" https://token.actions.githubusercontent.com v4.0.0-rc.5
old_identity=https://github.com/mrchypark/kubernetes-secret-generator/.github/workflows/workflow.yml@refs/tags/v4.0.0-rc.5
if sh scripts/verify-signing-identity.sh "$old_identity" https://token.actions.githubusercontent.com v4.0.0-rc.5 >/dev/null 2>&1; then
