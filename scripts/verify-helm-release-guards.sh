...
case "$1" in
	...
	package)
		while [ "$#" -gt 0 ]; do
			if [ "$1" = --destination ]; then shift; touch "$1/ksg-4.0.0-rc.13.tgz"; break; fi
			shift
		done
		;;
	show)
...
common_env() {
	env PATH="$tmpdir/bin:$PATH" CALL_LOG="$log" REAL_OPENSSL="$real_openssl" MOCK_SPEC_HASH="$tmpdir/spec-hash" \
		...
		NAMESPACE=ksg-system RELEASE_NAME=ksg DEPLOYMENT_NAME=ksg CHART_VERSION=4.0.0-rc.13 \
...
: >"$log"
if common_env MOCK_RELEASE_EXISTS=true MOCK_OWNER_RECORD='ksg-system|owner|flux|ksg-system|ksg|4.0.0-rc.13|ownNamespace||||\n' \
...
: >"$log"
common_env MOCK_CRD_VERSION=4.0.0 MOCK_OWNER_RECORD='ksg-system|owner|direct|ksg-system|ksg|4.0.0-rc.13|ownNamespace||||\n' \
...
persisted_owner='ksg-system|owner|direct|ksg-system|ksg|4.0.0-rc.13|ownNamespace||issue#23|@mrchypark|initial#22\n'
: >"$log"
