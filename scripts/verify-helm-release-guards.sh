...
case "$1" in
	...
	package)
		while [ "$#" -gt 0 ]; do
			if [ "$1" = --destination ]; then shift; touch "$1/ksg-4.0.0-rc.10.tgz"; break; fi
			shift
		done
		;;
	show)
...
common_env() {
	env PATH="$tmpdir/bin:$PATH" CALL_LOG="$log" REAL_OPENSSL="$real_openssl" MOCK_SPEC_HASH="$tmpdir/spec-hash" \
		...
		NAMESPACE=ksg-system RELEASE_NAME=ksg CHART_VERSION=4.0.0-rc.10 \
...
: >"$log"
if common_env MOCK_RELEASE_EXISTS=true MOCK_OWNER_RECORD='ksg-system\towner\tflux\tksg-system\tksg\t4.0.0-rc.10\townNamespace\t\n' \
...
: >"$log"
common_env MOCK_CRD_VERSION=4.0.0 MOCK_OWNER_RECORD='ksg-system\towner\tdirect\tksg-system\tksg\t4.0.0-rc.10\townNamespace\t\n' \
