...
export IMAGE_DIGEST='sha256:<64-hex-digest-from-v4.0.0-rc.17-candidate>'
...
`v4.0.0-rc.17` is a digest-addressed candidate for amd64 and arm64. It carries no SLA,
...
v4 runs exactly one controller Pod. Leader election, HA, HPA, manual scaling above one, and
multiple releases with overlapping watch scope are unsupported. v3 migration and rollback
are explicit downtime operations.
