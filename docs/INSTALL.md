...
The `v4.0.0-rc.7` candidate targets Kubernetes 1.34/1.35 APIs, Helm 3.14+, amd64,
...
export CHART_VERSION=4.0.0-rc.7
...
Existing Flux installations may use the indexed [Flux example](examples/flux-helmrelease.yaml) as a starting point. Replace its zero Git commit and image digest with the exact candidate; the Git source uses `reconcileStrategy: Revision`. Keep Flux as the sole CRD manager. Flux is optional and its rehearsal is not a universal rc.7 promotion blocker.

Choose replica and topology settings for the target cluster. The rc.7 release process does
...
Flux compatibility is documented for existing consumers but is not exercised by the rc.7
release automation.
...
The report must be no older than 24 hours and match the current context, server, CA, namespace, and release. The three unmarked CRDs must byte-match the pinned v3.4.1 normalized specs; partial or unknown CRD sets are rejected before write. Immediately before adoption the wrapper reruns preflight, rejects non-`kubectl-client-side-apply` managed fields, and replaces each exact CRD with its captured UID/resourceVersion so concurrent changes fail. It then establishes normal non-forcing SSA ownership. The CRDs are updated in place and retained for manager rollback; marked v4 CRDs and Flux-managed installations never use forced ownership.
