# v4.0.0-rc.6 release process

`v4.0.0-rc.6` is a candidate, not an SLA or capacity claim. Promotion is a manual,
maintainer-owned decision recorded in a GitHub issue and protected by the repository's
GitHub Environment.

## Candidate build

1. Prepare the annotated `v4.0.0-rc.6` tag from the intended source commit, without pushing
   it yet. Confirm `Chart.yaml` version/appVersion and `values.yaml` image tag match rc.6.
